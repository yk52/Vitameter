...
class Values {
	public:
		...
		// Ram Storage arrays
		uint16_t co2[3] = {0};  // in ppm. in flash memory, its divided by 100 to fit in a byte
		uint16_t co2_idx;
		uint8_t voc[3] = {0}; // in ppb, bzw. mg/m3
		uint16_t voc_idx;
		uint8_t uvi[3] = {0};   // UV index
		uint16_t uvi_idx;
		float temp[3] = {0}; // in degrees
		uint16_t temp_idx;
		uint16_t steps;
